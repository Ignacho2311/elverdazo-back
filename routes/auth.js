...
check("username","El nombre de usuario es requerido").not().isEmpty()],[validationErrors], registerUsuario)
authRouter.post("/login", [check("email","El formato es invalido").isEmail(),
check("password","La contraseña tiene que ser de 6 caracteres minimo").isLength({min:6})] ,[validationErrors], loginUsuario)
